...
test('character image generation uses 4K 16:9 asset size', () => {
  ...
  assert.match(scenes, /SCENE_IMAGE_SIZE = '3840x2160'/)
  assert.match(scenes, /size: body\.size \|\| SCENE_IMAGE_SIZE/)
  ...
  assert.match(geminiImage, /if \(!size\) return '4K'/)
  assert.match(geminiImage, /if \(!w\) return '4K'/)
})
